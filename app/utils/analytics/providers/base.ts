 ...
 */
import type { AnalyticsProvider, ProviderConfig, ProviderResult } from "../types";
...
export abstract class BaseProvider implements Partial<AnalyticsProvider> {
   ...
   */
  protected async executeRequest(requestFn: () => Promise<Response>): Promise<ProviderResult> {
  ...
  protected validateCredentials(required: string[]): void {
    ...
    if (missing.length > 0) {
      throw new Error(`${this.name} missing required credentials: ${missing.join(", ")}`);
    }
  }
}
